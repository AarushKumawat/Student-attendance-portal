...
const isTeacherAuthenticated = async (
  ...
  try {
    const query = `SELECT * FROM teacher_token WHERE token= ?`;
    ...
    const userId = data[0][0].fk_teacher;
    const userQuery = `SELECT * FROM teacherinformation WHERE id = ?`;
...
const generateTeacherToken = async (user_id) => {
  try {
    console.log(user_id);
    ...
    const tokenRecord = `INSERT INTO teacher_token(token, fk_teacher, created_at) VALUES(?, ?, ?)`;
...
module.exports = {generateTeacherToken, isTeacherAuthenticated, isStudentAuthenticated, generateStudentToken
}
